...
@patch('factories.db_factory.SQLDatabase')
def test_create_db(mock_sql_db):
    DBFactory.create_db()
    mock_sql_db.from_uri.assert_called()

@patch('factories.db_factory.create_sql_agent')
@patch('factories.db_factory.LLMFactory.create')
@patch('factories.db_factory.DBFactory.create_db')
@patch('factories.db_factory.SQLDatabaseToolkit')
